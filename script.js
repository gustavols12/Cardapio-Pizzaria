...
function addtocart(name, price) {
  // verificação se item já existe no carrinho
  const duplicItem = cart.find((item) => item.name === name);
  if (duplicItem) {
    duplicItem.quantity += 1;
  } else {
    cart.push({
      name,
      price,
...
// descobrindo a posição do item que vai ser removido
...
// verificando se o endereço está vazio
